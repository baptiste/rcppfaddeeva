...
-->
The Voigt lineshape is extremly important in spectroscopy. The direct approach of numerical convolution between a Lorentzian (width $\gamma$) and a Gaussian (width $\sigma$) can be slow, and requires care to obtain sufficient accuracy if the width of the two functions is very different. In contrast, the Faddeeva function offers a straight-forward, reliable, and accurate calculation for any combination of $\gamma$ and $\sigma$. With the `RcppFaddeeva` package we leverage in `R` the elegant `c++` routines provided by Steve G. Johnson, with seamless bindings provided by `Rcpp`. 
...
An interesting side-effect of obtaining the Voigt lineshape from the imaginary part of the complex Faddeeva function is the direct access to its harmonic conjugate via the real part. This can be useful in modelling response functions linked by a Hilbert transform.
...
legend("topleft", legend = c("Real part", "Imaginary part"), bty="n",
      lty=c(1,2), col=1)
```
